...
subZ = read_csv("subPropTestVolc.csv")
subOdds = read_csv("subEffectForVolc.csv")
subData = inner_join(subZ, subOdds, by=c("SNP", "risk allele", "population1", "population2"))

ggplot(data=subData, aes(x=subData$`odds ratio`, y=subData$p_value)) + geom_point()

newData = mutate(subData, logOdds = log2(`odds ratio`))
de = newData
# add a column of NAs
...
ggplot(data=de, aes(x=logOdds, y=-log10(p_value), col=diffexpressed, label=delabel)) +
  ...
  ylab("-log10(p-value)") +
  ggtitle("Subpopulation Volcano Plot") +
